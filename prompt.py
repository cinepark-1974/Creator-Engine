...
def _is_comedy(genre: str) -> bool:
    """장르가 코미디인지 판별"""
    return "코미디" in genre.lower() or "comedy" in genre.lower()
...
def build_system_core(genre: str) -> str:
    ...
    comedy_block = COMEDY_RULES if _is_comedy(genre) else ""
...
def build_system_char_bible(genre: str, fmt: str, others_str: str) -> str:
    """Character Bible 시스템 프롬프트 — Tactics = Character + LOCKED + 코미디 특화"""
    comedy_char_block = ""
    if _is_comedy(genre):
        comedy_char_block = """
...
def build_system_scene_design(genre: str) -> str:
    """Scene Design v1.9 — Drop in the Middle + Hook/Punch + 관객 심리 + 코미디 특화 + LOCKED"""
    genre_rules = get_genre_rules(genre)
    comedy_scene_block = ""
    if _is_comedy(genre):
        comedy_scene_block = """
...
- setpiece는 '대형 코믹 셋피스'여야 한다. 스릴러 셋피스가 아니라 웃음이 폭발하는 장면.
- key_line은 건조하고 짧은 코믹 대사. 설명하지 않는 대사.
"""
    genre_rules = get_genre_rules(genre)
...
def build_system_treatment(genre: str, act_label: str, fmt: str = "", b_story_context: str = "") -> str:
    """Treatment v1.9 — SCOPE MANDATE + 코미디 특화 + B-Story + LOCKED"""
    genre_rules = get_genre_rules(genre)
    genre_rules_text = json.dumps(genre_rules, ensure_ascii=False)
    is_series = is_series_format(fmt)

    comedy_treatment_block = ""
    if _is_comedy(genre):
        comedy_treatment_block = """
[코미디 트리트먼트 특화 — COMEDY NARRATIVE OVERRIDE]
★ 이 비트는 코미디다. narrative 줄글 자체가 웃겨야 한다. ★
★ 구조만 코미디이고 서술이 드라마이면 실패다. 서술 문체 자체를 바꿔라. ★

[narrative 서술 문체 — 코미디 전용]
1. 행동 묘사에 물리적 디테일을 넣어라 — 추상적 행동 금지.
   ❌ 드라마체: '재하가 계산대에 앉는다. 긴장감이 감돈다.'
   ✅ 코미디체: '재하가 계산대에 앉자마자 POS기 화면이 얼어붙는다. 모니터를 두드린다. 세 번째에 뜨는 건 윈도우 업데이트 화면이다.'
   → 물건이 고장나고, 넘어지고, 끼이고, 울리고, 쏟아지는 물리적 사건이 웃음을 만든다.

2. 한 문장 안에 기대와 결과를 넣어라 — '~했는데 ~였다' 구조.
   ❌ '재하가 문제를 해결한다.'
   ✅ '재하가 3분 만에 민원을 해결한다 — 그런데 해결한 순간 뒤에서 새 민원 네 개가 동시에 터진다.'
   → 성공 직후 실패, 해결 직후 악화. 이것이 코미디의 기본 리듬이다.

3. 시간을 구체적으로 써라 — 숫자가 웃기다.
   ❌ '오래 걸렸다.'
   ✅ '5분이면 끝난다고 했다. 47분이 지났다.'
   → 구체적 숫자의 격차가 웃음을 만든다. '많이' '오래' '엄청' 같은 추상 부사 금지.

4. 대사는 짧고 건조하게 — 상황이 대사를 웃기게 만든다.
   ❌ (재하가 화를 내며) '아니 이게 말이 돼요? 사장님이 약속을 어기시면 제가 어떻게 해요!'
   ✅ (주방이 거품으로 뒤덮인 상태에서) 재하가 말한다. '환기가 필요하겠네요.'
   → 캐릭터는 감정을 말하지 않는다. 상황이 감정을 보여준다. 대사는 상황과 어긋날수록 웃기다.

5. 빌런의 대사/행동은 '자기가 옳다고 진심으로 믿는' 톤으로.
   ❌ 빌런이 악의적으로: '너를 이용할 거야.'
   ✅ 빌런이 진심으로: '내가 이렇게 기회를 주는 사장이 어디 있어. 감사해야지.'
   → 빌런이 진심이기 때문에 웃기다. 관객은 아는데 빌런만 모른다(dramatic irony의 코믹 버전).

6. 에스컬레이션 — 같은 패턴이 반복되는데 매번 더 심해지게 써라.
   씬1: 민원 1개 해결 → 씬2: 민원 3개 동시 → 씬3: 민원 7개 + 화재 + 정전
   → 숫자/규모/속도가 매 씬마다 올라가야 한다. 평탄하면 코미디가 아니다.

7. 코믹 이미지로 비트를 끝내라 — 마지막 문장이 시각적으로 웃겨야 한다.
   ❌ '재하는 결심한다.'
   ✅ '재하가 정장 위에 앞치마를 두르고 고무장갑을 낀다. 거울에 비친 자신을 본다. 이력서 사진과 현재의 격차가 우주적이다.'
   → 비트의 마지막 이미지가 코믹 이미지여야 관객이 웃으면서 다음 비트로 넘어간다.

[매 비트 필수 체크]
□ 이 비트의 코믹 엔진은 무엇인가? (상태 역전 / 에스컬레이션 / 극과 극 / 관객 우월감 중 택1)
□ narrative 안에 관객이 웃는 구체적 순간이 최소 3개 있는가?
□ 대사가 건조하고 짧은가? 상황이 대사를 웃기게 만드는가?
□ 대사에 코미디 7기법 중 최소 1개가 쓰였는가?
□ 빌런/장애물 인물이 '진심으로 자기가 옳다고 믿는' 톤인가?
□ 숫자/시간이 구체적인가? ('오래' 대신 '47분')
□ 비트 마지막 문장이 코믹 이미지인가?
□ 거짓말/오해/착각이 이 비트에서 더 커졌는가? (눈덩이 원칙)
□ 코미디의 stakes는 수치(embarrassment)인가? (목숨이 아니라 자존심)
"""
