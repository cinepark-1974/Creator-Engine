...
SUBTITLE_VISIBILITY_GUARD = """
[★★★ v2.5.6 — 자막 가시성 강제 룰 (절대 위반 금지) ★★★]

이 룰은 모든 톤 지시보다 상위다. 톤 디자이너가 작성하는 모든 dialogue 룰은
다음 자가검증을 통과해야 한다.

1. 시(詩) 금지 — 구체 구현으로:
   ❌ "말하지 않는 것이 말하는 것보다 많아야 한다" (작가가 페이지에 못 옮김)
   ✅ "감정 명명 대사는 110분 전체에서 1회로 제한, 나머지는 행동·공간 반응으로 전달"
   ❌ "대사 표면 아래 깊이가 흐른다"
   ✅ "Reza의 애정 표현 대사 직후 통제 행동(문 잠그기·동선 차단)을 지문에 명시"

2. 자막에서 사라지는 정보 금지:
   언어학적 미묘함(존대 체계·고어체·외국어 어순)은 한국 OTT 자막에서 동일하게 번역되어
   사라진다. 톤 룰이 언어 자체에 의미를 의존하면 관객이 인지하지 못한다.
   - 외국어 대사가 의미를 운반하는 모든 순간에는 동석 인물의 즉시 반응 지문이 동반되어야 한다.
   - 존대 체계의 변화는 호칭·자리·동선·시선 같은 시각 신호로도 동시 표현되어야 한다.

3. Writer 구현 가능성 자가검증:
   각 dialogue 룰을 작성한 후, 다음 질문에 답할 수 있어야 한다.
   - "이 룰을 시나리오 페이지에 어떻게 박을 것인가?" (지문·반응·동선 단위로 답할 것)
   - "이 룰이 한국 자막 시청자에게 도달하는가?"
   - "관객이 의식하지 못한다면 그것은 룰의 작동인가, 부재인가?"
...
4. 반응 앵커(Reaction Anchor) 강제:
   ...
   - 반응이 없으면 관객은 '결이 바뀌었다'를 인지할 수 없다.
   - 이 원칙은 dialogue_rules.reaction_anchor_rule 항목에 명시할 것.
"""


MULTILINGUAL_TONE_GUARD = """
[★★★ v2.5.6 — 다국어 작품 추가 가드 (해당 작품에만 자동 주입) ★★★]

이 작품은 다국어 작품으로 자동 감지되었다. 한국 OTT 자막을 거치는 모든 외국어 대사는
다음 추가 룰을 통과해야 한다.

1. 외국어 대사 = 자막 대사 전제:
   인도네시아어·자바어·네덜란드어·영어·일본어·중국어 등 모든 외국어는
   한국 자막에서 동일한 한국어로 번역된다. 언어 종류 자체에서 발생하는 결
   (halus/kasar, 고어체, 식민지 잔재 어휘)은 자막에서 사라진다.

2. 외국어 대사가 의미를 운반할 때 강제 사항:
   - 지문에 언어 명시 의무: '(자바어로)' '(네덜란드어로)' '(인도네시아어 halus)' 등
   - 동석 인물 반응 지문 동반 의무: 흠칫·뒷걸음·시선 이동·기도 동작·정적 등
   - 의미 운반이 핵심인 경우 시각 단서 동반 의무: 손동작·문양·물증·표정 변화

3. 외국어 가사·주문·문서가 서사 정보를 운반할 때:
   - 자막 번역만으로 끝내지 말 것
   - 그 의미를 듣는 캐릭터의 표정·행동·필사·정지 동작을 지문에 동시 배치
   - 관객은 외국어를 몰라도 캐릭터의 반응만으로 의미를 해독할 수 있어야 한다

4. 외국어 존대 체계·고어체 변화는 시각 신호로 동시 표현:
   누가 먼저 앉는가, 누가 차를 따르는가, 호칭이 어떻게 바뀌는가
   — 이 시각 신호가 자막에서 살아남는 위계 정보다.
...
def build_system_tone_document(genre: str, fmt: str,
    ...
    v2.4.0: locked_text/idea_text 추가 (시대극 자동 감지용)
    v2.5.6: 자막 가시성 가드 + 다국어 작품 자동 감지 가드 추가
    """
    fact_block = get_fact_based_rules(fact_based)
    historical_block = get_historical_film_rules(historical, film_type, locked_text=locked_text, idea_text=idea_text)
    # v2.5.6 — 다국어 작품 자동 감지
...
TONE_DOC_SCHEMA = """{
    ...
    "dialogue_rules": {
        "overall_tone": "대사 전체 톤 1~2문장 — ★ v2.5.6: 자막에서도 살아남는 구체적 표현 강제. '말하지 않는 것이 말하는 것보다 많다' 같은 시(詩) 금지. 호칭·자리·동선·반응 지문 등 자막을 거치지 않는 시각 신호로 위계·관계를 표현하는 방식을 명시할 것.",
        "subtext_rule": "서브텍스트 규칙 2~3문장 — ★ v2.5.6: Writer가 페이지에 박는 방법(지문·반응·동선)을 반드시 명시. '관객이 후반에 깨닫는다'식 추상 금지. 어느 비트에서 어떤 행동·동선·소품을 통해 서브텍스트가 드러나는지 구체화. 표면 대사와 행동의 어긋남을 어떻게 누적시키는지 1막부터 페이지 단위 구현 방식을 적시.",
        "silence_usage": "침묵/비언어 활용 규칙 1~2문장 — 침묵의 길이·환경음·카메라가 무엇을 비추는가까지 명시할 것.",
        "too_wet_guard": "Too Wet 방지 규칙 2~3문장 — ★ v2.5.6: 감정 명명 금지에 더해 '대신 무엇으로 전달하는가'를 반드시 명시. 행동(손떨림·문 잠그기·반복 동작) / 공간 반응(벽 균열·촛불 꺼짐·물건 떨어짐) / 신체 반응(호흡·시선·자세) 중 어떤 채널을 사용하는지, 감정 명명 예외(작품 전체에서 몇 회 허용)는 몇 회인지 적시.",
        "reaction_anchor_rule": "★ v2.5.6 신규 — 반응 앵커(Reaction Anchor) 룰 2~3문장. 외국어 전환·서브텍스트·은유·언어학적 변화 등 비표면적 정보가 의미를 운반하는 모든 순간에 동석 인물의 즉시 반응 지문(흠칫·뒷걸음·시선 회피·정적·기도 동작·한 박자 늦은 답 등)을 반드시 동반시켜야 한다는 룰을 작품별로 구체화. 어느 비트에서 누가 무엇을 반응하는가의 패턴을 명시. ★ 다국어 작품·서브텍스트 중심 작품에서 특히 중요. ★",
        "forbidden_phrases": ["이 작품에서 절대 쓰지 않을 대사 패턴 3~5개 — ★ v2.5.6: 다국어 작품의 경우 '외국어만으로 의미를 운반하는 대사 금지' 룰을 반드시 1개 포함시킬 것."]
    },
    "motifs": {
...
narrative는 트리트먼트에서 가장 긴 string value다 (2500~4000자).
