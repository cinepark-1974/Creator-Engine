...
def build_system_treatment(genre: str, act_label: str) -> str:
    """Treatment v1.4 — Unified Plot Test + Too Wet + 관객 심리 4규칙"""
    genre_rules = get_genre_rules(genre)
...
[관객 심리 설계 — v1.4]
12. Dramatic Irony: 관객에게 캐릭터가 모르는 정보를 먼저 줘라. 그 정보가 있는 상태에서 캐릭터의 행동을 서술하라.
13. Delayed Gratification: 관객이 가장 원하는 장면(대결/재회/진실)을 최대한 늦춰라. 기다림이 보상을 키운다.
14. Pattern & Violation: 반복되는 패턴을 세팅하고, 핵심 비트에서 깨뜨려라.
15. Information Gap: 매 비트에서 최소 1개의 열린 질문을 유지하라. 답을 주면 새 질문을 열어라.
...
# ─── Treatment Beat 스키마 (v1.4: hook/punch + dramatic_irony/open_question/subplot_progress) ───
