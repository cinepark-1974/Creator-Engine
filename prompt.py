...
def _get_genre_override(genre: str) -> str:
    """장르별 특화 규칙 통합 반환. 롬코면 코미디+로맨스 모두."""
    blocks = []
    if _is_comedy(genre):
...
def build_system_core(genre: str) -> str:
    ...
    comedy_block = _get_genre_override(genre)
...
def build_system_char_bible(genre: str, fmt: str, others_str: str) -> str:
    """Character Bible 시스템 프롬프트 — Tactics = Character + LOCKED + 장르 특화"""
    genre_char_block = ""
    if _is_comedy(genre):
        genre_char_block += """
...
def build_system_scene_design(genre: str) -> str:
    """Scene Design v1.9 — 장르별 특화 + Hook/Punch + LOCKED"""
    genre_rules = get_genre_rules(genre)
    genre_scene_block = ""
    if _is_comedy(genre):
        genre_scene_block += """
...
[AI 안티패턴 — narrative에서 절대 금지]
A1. 감정 설명 서술 금지: '불안한 마음으로' '두려움이 온몸을' → 행동으로 보여줘라. '손잡이를 잡았다 놓았다.'
A2. 방금 서술한 것을 대사로 반복 금지: 관객은 이미 읽었다. 반응을 보여줘라.
A3. 총칭적 감각 금지: '바람이 불었다' '어둠이 깔렸다' → '커튼이 빨려 들어간다. 목덜미에 소름.'
A4. 같은 씬에서 긴장 생성+해소 금지: 긴장을 씬 경계 너머로 끌고 가라.
