"""
👖 BLUE JEANS Creator Engine v1.3 — Prompt Library
All system prompts, user prompt templates, and Sorkin/Curtis principles.
...
# Character Bible JSON 스키마 (tactics 추가)
CHAR_BIBLE_SCHEMA = """{
...
def build_system_structure_story() -> str:
    """Structure Build Story 시스템 프롬프트"""
...
def build_system_structure_diagnosis() -> str:
    """Structure Diagnosis 시스템 프롬프트 — Probable Impossibility 검증"""
    return """당신은 헐리우드 수준의 Structure Analyst다.
시놉시스와 스토리라인을 기반으로 구조 진단과 캐릭터 변화를 설계한다.

중요 규칙:
- 비트 검증 시 각 비트에 대해 Probable Impossibility 체크를 수행한다.
- '억지 우연으로 해결되는 비트가 있는가?' → 있으면 해당 비트의 status를 '약함'으로 표시하고 note에 '우연 의존 — Probable Impossibility 위반' 명시.
- '이 비트를 빼면 이야기가 작동하는가?' → 작동하면 해당 비트의 status를 '약함'으로 표시하고 note에 'Unified Plot Test 미통과 — 삭제/병합 고려' 명시.
...
def build_system_scene_design(genre: str) -> str:
    """Scene Design 시스템 프롬프트 — Drop in the Middle + Hook/Punch"""
    genre_rules = get_genre_rules(genre)
...
def build_system_treatment(genre: str, act_label: str) -> str:
    """Treatment 시스템 프롬프트 — Unified Plot Test + Too Wet 금지"""
    genre_rules = get_genre_rules(genre)
...
11. HOOK과 PUNCH가 없는 비트는 존재 이유가 없다.
...
# ─── Treatment Beat 스키마 (hook/punch 포함) ───
...
- unified_plot_test는 매 비트 필수. 'N'이어야 한다. 'Y'가 나오면 그 비트를 더 강하게 재설계하라.
- Too Wet 금지: '그는 슬펐다', '분노가 치밀었다' 같은 감정 직접 서술 금지. 행동과 선택으로 보여줘라."""

# ─── Scene Design 스키마 (hook/punch/setpiece 포함) ───
SCENE_DESIGN_SCHEMA = """{
  "key_scenes": [
    {
      "scene_no": 1,
