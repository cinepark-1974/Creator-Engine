...
def build_system_treatment(genre: str, act_label: str, fmt: str = "", b_story_context: str = "") -> str:
    ...
    if _is_comedy(genre):
        comedy_treatment_block = """
[코미디 트리트먼트 특화 — COMEDY TREATMENT OVERRIDE]
★ 이 비트는 코미디다. 매 비트에 웃음이 있어야 한다. 사건만 있고 웃음이 없으면 스릴러다. ★
...
[코미디 비트 서술 규칙]
- 웃음의 원천을 명시하라: 이 비트에서 관객이 웃는 이유가 대사인가, 상황인가, 타이밍인가.
- 상황 코미디 > 대사 코미디. 상황이 웃기면 대사는 건조해도 된다.
- 매 비트에 최소 2개의 코믹 모먼트(관객이 웃는 순간)를 설계하라.
- 코미디의 stakes는 수치(embarrassment)다. '면접 놓침', '비밀 들킴', '체면 무너짐' — 목숨이 아니라 자존심.
- 2막 끝: 웃음이 멈추는 순간(Moment of Truth). 코믹 결함 뒤의 진짜 아픔이 드러난다.
"""
