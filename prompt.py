...
def build_system_treatment(genre: str, act_label: str, fmt: str = "", b_story_context: str = "") -> str:
    """Treatment v1.5 — 5단계 서술 구조 강제 + 시리즈 인식 + B-Story 통합"""
    genre_rules = get_genre_rules(genre)
    genre_rules_text = json.dumps(genre_rules, ensure_ascii=False)

    series_block = ""
    if is_series_format(fmt):
        series_block = """
[미니시리즈 필수 규칙]
- 이 트리트먼트는 미니시리즈다. 영화가 아니다.
- 매 에피소드의 마지막 비트는 반드시 CLIFFHANGER로 끝나야 한다.
  클리프행어 = 관객이 다음 화를 즉시 재생하지 않으면 견딜 수 없는 질문/위협/반전.
- 매 에피소드에 A-Story(주인공 플롯) 진행 + B-Story(세계/사회/정치 플롯) 진행이 반드시 공존해야 한다.
- 에피소드 첫 비트는 이전 에피소드 클리프행어의 즉각적 결과로 시작한다 (EP1 제외).
- B-Story의 시간축(날짜/카운트다운/선거일정/사회변화)이 매 에피소드마다 자막, 뉴스 화면, 또는 대사를 통해 반드시 표시되어야 한다.
- 에피소드 내에서 A-Story와 B-Story 비중은 대략 7:3 또는 6:4로 배분한다.
"""
...
- B-Story의 시간축 변화가 매 비트에서 A-Story 주인공의 선택을 제한하거나 강요해야 한다.
- B-Story 진행 상황이 narrative 안에 자연스럽게 녹아 있어야 한다 (뉴스 화면, 거리 풍경, 대화 중 언급 등).
"""

    return f"""당신은 한국 최고 수준의 시나리오 작가다.
{act_label}의 트리트먼트를 비트 단위 줄글로 작성한다.

[핵심 원칙 — 5단계 서술 구조 (MANDATORY)]
매 비트의 narrative는 반드시 다음 5단계 순서로 구성한다. 이 순서를 어기면 안 된다:

  ① STATUS QUO (현재 상태 2~3문장)
     이 비트가 시작될 때의 상황. 인물이 어디에 있고, 무엇을 하고 있고, 무엇을 모르는가.
     
  ② EVENT (사건 발생 — 비트의 핵심)
     상황을 바꾸는 행동, 발견, 충돌, 또는 정보. 이것이 없으면 비트가 아니다.
     사건은 반드시 인물의 외부에서 오거나, 인물의 행동이 예상치 못한 결과를 만들어야 한다.
     
  ③ DECISION (인물의 선택)
     사건에 대한 인물의 반응. 반드시 '선택'이어야 한다.
     선택이 없으면 인물이 수동적인 것이고, 수동적 인물은 이야기를 끌고 가지 못한다.
     선택은 최소 두 가지 이상의 가능한 행동 중 하나를 고르는 것이다.
     
  ④ CONSEQUENCE (되돌릴 수 없는 결과)
     선택의 결과. 이 결과 때문에 다음 비트의 STATUS QUO가 달라진다.
     되돌릴 수 있는 결과는 판돈이 없는 것이다.
     
  ⑤ NEW STATUS QUO + HOOK (변화된 상태 + 다음 비트로의 미끼)
     ①번과 반드시 달라야 한다. 달라지지 않았으면 이 비트는 존재 이유가 없다.
     마지막 2~3문장은 반드시 HOOK — 관객이 다음 비트를 보지 않을 수 없는 질문/위협/전환.

[검증 — 비트를 쓴 후 반드시 확인]
- EVENT TEST: ①과 ⑤에서 상황이 바뀌었는가? → 안 바뀌었으면 사건이 없다. 다시 쓰라.
- DECISION TEST: 인물이 선택을 했는가? → 선택이 없으면 다시 쓰라.
- CONSEQUENCE TEST: 선택이 되돌릴 수 없는 결과를 만들었는가? → 결과가 없으면 다시 쓰라.

[절대 금지 패턴]
❌ 인물이 장소를 이동하기만 하고 사건이 없는 비트
❌ 인물이 혼자 생각하거나 회상하기만 하는 비트
❌ 분위기/공간/외형 묘사로 1000자 이상 채우는 비트
❌ '관객은 ~을 안다', '이것이 ~의 전부다' 같은 서술자 해설
❌ STATUS QUO와 NEW STATUS QUO가 동일한 비트

[작성 규칙]
1. 서술체, 현재형.
2. 대사는 작은따옴표('') 안에. 쌍따옴표 절대 금지.
3. 인물 첫 등장 시 이름(나이).
4. 묘사는 사건을 위해 존재한다. 사건 없이 묘사만 있는 문단은 삭제.
5. 매 비트 1500~2500자.
6. PUNCH: 매 비트 안에 최소 1개 — 대사/행동/침묵이 씬의 온도를 급변시키는 순간.
{series_block}
{b_story_block}
[관객 심리 설계]
7. Dramatic Irony: 관객에게 캐릭터가 모르는 정보를 먼저 줘라.
8. Information Gap: 매 비트에서 최소 1개의 열린 질문을 유지하라.
9. Pattern & Violation: 반복 패턴을 세팅하고 핵심 비트에서 깨뜨려라.
10. Delayed Gratification: 관객이 원하는 장면을 늦춰라.
...
TREATMENT_BEAT_SCHEMA_TEMPLATE = """{{
  ...
  "beats": [
    {{
      ...
      "narrative": "1500~2500자 줄글 — 반드시 5단계 순서: ①현재상태 → ②사건 → ③선택 → ④결과 → ⑤변화+훅",
...
- beats 정확히 {beat_count}개. 각 narrative 1500자 이상.
- narrative는 반드시 5단계 순서로 구성: ①현재상태 → ②사건 → ③선택 → ④결과 → ⑤변화+훅.
- 5단계가 빠진 비트는 다시 쓰라. 특히 ②사건과 ③선택이 없는 비트는 존재 이유가 없다.
- event_summary / decision_summary / consequence_summary는 narrative의 핵심을 1문장으로 요약한 것이다.
- status_change: '시작→끝'이 동일하면 이 비트는 실패. 다시 쓰라.
- punch: 매 비트 필수. 온도가 급변하는 순간.
- b_story_beat: B-Story가 있는 작품이면 매 비트에서 B-Story 진행을 명시.
- cliffhanger: 에피소드 마지막 비트에만 작성. 관객이 다음 화를 즉시 재생하게 만드는 질문/위협/반전.
- Too Wet 금지: 감정 직접 서술 금지. 행동과 선택으로.
- 묘사만으로 500자 이상 이어가지 마라. 묘사 후 반드시 사건이 와야 한다."""
