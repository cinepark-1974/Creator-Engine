# 👖 BLUE JEANS · Creator Engine v1.9
...
③ Core Build (Logline Pack + GNS + 서사동력 + Plant 설계 + Gate B/C) [Sonnet]
④ Character Bible (백스토리 · 전술 · 말투 · 대사 · 변화궤적)    [Opus]
⑤ Structure Build (Synopsis + Storyline + 3막 진단 + 빌런 승률 + Gate D) [Sonnet]
⑥ Scene Design (핵심 장면 15~18개 · Plant/Payoff 태그 · 매력 설계도) [Sonnet]
⑦ Treatment Build (16비트 다중씬 트리트먼트 · Payoff 회수 체크)   [Opus]
...
### Planting & Payoff (v1.9)
```
③ Core Build    → planting_payoff 3쌍 설계 (character / relationship / world)
⑥ Scene Design  → 각 장면에 plant_payoff_tag (심기/회수 태그)
⑦ Treatment     → 각 비트에 plant_payoff 체크 (회수 없는 Plant = 불만족, Plant 없는 Payoff = 금지)
```
- 1막에 심은 것이 3막에서 새로운 의미로 회수
- 최소 3쌍: 캐릭터 Plant · 관계 Plant · 세계관 Plant
...
## Foundation — 헐리우드 표준 작법 (17개 원칙)

### Sorkin/Curtis 9원칙
BUT/EXCEPT 테스트 · Intention & Obstacle · Tactics = Character · Probable Impossibility · Drop in the Middle · Unified Plot Test · Too Wet 금지 · Curtis 3% 법칙 · Curtis 감정 연쇄

### 관객 심리 6원칙
Dramatic Irony · Information Gap · Zeigarnik Effect · Pattern & Violation · Delayed Gratification · Mystery Box

### 서브플롯 + Planting & Payoff
서브플롯 설계 원칙 · **Planting & Payoff** (v1.9 신규)
...
```
creator-engine/
├── main.py              4,755줄  UI + API + JSON 파서 + DOCX 생성
├── prompt.py            1,247줄  프롬프트 라이브러리 (3축 + Foundation 17원칙 + LOCKED)
...
| **v1.9** | **Planting & Payoff 시스템 · 기능적 조연 배치 · 서사동력 비트별 체크 · Foundation 17원칙** |
